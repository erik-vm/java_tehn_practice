...
public class SpringTest {
    ...
    public void constructProductDaoWithPropertiesOfTheSameType() {
        ...
        assertThat(dao.getKey2(), is("key 2 value"));
    }

    @Test
    @Points(1)
    public void dao2IsAvailableWithProfile2() {
        // should not throw

        getContextWithProfile("p2").getBean(ProductDao2.class);
    }
    ...
    public void constructJustOneMatchingBean() {
        ...
        getContextWithProfile("p3").getBean(Dao.class);
    }

    @Test
    @NoPointsIfThisTestFails
    public void configDescribesDataSourceAndString() {
        var returnTypes = Arrays.stream(Config.class.getDeclaredMethods())
                .map(Method::getReturnType).toList();

        var allowed = List.of(DataSource.class, String.class);

        for (Class<?> returnType : returnTypes) {
            assertTrue("Config.java should not define how to build '"
                    + returnType + "'",  allowed.contains(returnType));
        }
    }
